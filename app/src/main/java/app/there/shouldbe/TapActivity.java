...
//    private OnInfoWindowElemTouchListener likeButtonListener; 
    ...
    private static final String PREF_SEARCHED = "last_searched";
    private static final String PREF_ADDRESS_LAT = "address_lat";
    private static final String PREF_ADDRESS_LONG = "address_long";
        ...
        searchButton.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (!searchString.isEmpty()) {
					SearchClicked sc = new SearchClicked(mapSearchBox.getText().toString());
...
//                likeButton.setOnTouchListener(likeButtonListener);
    ...
    public void shouldBeUpdate (String status) {
    	...
    	//if last struct of the temp marker array != null
    ...
    /**
     * Returns true if the last address was searched
     * @return true if searched, false otherwise
     */
    private boolean searched() {
    	return mSharedPreferences.getBoolean(PREF_SEARCHED, false);
    }
	...
	private class SearchClicked extends AsyncTask<Void, Void, long[]> {
		...
		protected long[] doInBackground(Void... voids) {
			...
			if (!toSearch.isEmpty()) {
				try {
					...
					while (i < 5 && !found) { // try 4 times to get the location
						results = geocoder.getFromLocationName(toSearch, 1);
						if (results.get(0) != null) // if an address was found
							found = true;
						i++;
					}
					if (results.size() > 0) {
						address = results.get(0);
						result[0] = (long) address.getLatitude();
						result[1] = (long) address.getLongitude();
					}
				}
				catch (Exception e) {
					e.printStackTrace();
		...
		@Override
		protected void onPostExecute(long[] latlng) {
			LatLng l = new LatLng((int) (latlng[0] * 1E6), (int) (latlng[1] * 1E6));
